...
ui <- fluidPage(
  ...
  selectInput('x', 'Attribute of Interest', c("height", "dbh")),
  plotOutput("plot"),
)
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    rd <- reactive_data()
    ggplot(rd, aes_string(x = input$x)) +
      geom_histogram() +
      theme_bw() +
      ggtitle(paste("Histogram of", input$x)) +
      labs(x = paste(input$x, "distribution"),
           y = "frequency")
  })
}
...
Now that we have explored some data collection procedures and some simply visualizations, we want to see how well we did in capturing the variability in these forest attributes (height, DBH). To do that, we will compare our data with what professionals collected in the area! Let us have a look...
